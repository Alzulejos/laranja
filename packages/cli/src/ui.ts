...
export interface Choice<T> {
  label: string;
  value: T;
  /** Render as a dim, non-selectable section heading (cursor skips over it). */
  header?: boolean;
}
...
export function select<T>(title: string, choices: Choice<T>[]): Promise<T | undefined> {
  return new Promise((resolve) => {
    const { stdin, stdout } = process;
    // Headers are not selectable; start (and stay) on a real choice.
    const selectable = (n: number) => !choices[n].header;
    const step = (from: number, dir: number) => {
      let n = from;
      do {
        n = (n + dir + choices.length) % choices.length;
      } while (!selectable(n) && n !== from);
      return n;
    };
    let i = choices.findIndex((c) => !c.header);
    if (i < 0) i = 0; // degenerate: nothing selectable
    ...
    const onData = (buf: Buffer) => {
      const key = buf.toString();
      if (key === "\x1b[A" || key === "k") i = step(i, -1);
      else if (key === "\x1b[B" || key === "j") i = step(i, 1);
      else if (key === "\r" || key === "\n") {
        cleanup();
