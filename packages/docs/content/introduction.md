...
|---|---|
| HTTP API | An Express app, marked with the [`http()`](./reference/decorators-and-markers.md#http) marker (or named via `entry` in config). NestJS coming. |
...
| Per-environment deploys | [Stages](./concepts/stages-and-environments.md) (`--stage`) |
| Env vars | [`env`](./configuration/environment-variables.md) in config |
